...
void auto_tracking(void) {
	semaphore = 0;
	uint8_t sensor = 0;
	sensor = sensor | !digitalRead(LFSensor_0) << 4;

	sensor = sensor | !digitalRead(LFSensor_1) << 3;

	sensor = sensor | !digitalRead(LFSensor_2) << 2;

	sensor = sensor | !digitalRead(LFSensor_3) << 1;

	sensor = sensor | !digitalRead(LFSensor_4);
	if (sensor == 0b01000 || sensor == 0b10000 || sensor == 0b11000) {
...
//		SendStringUSB(" LEFT");
	...
	if (sensor == 0b01100 || sensor == 0b11100 || sensor == 0b11110) {
		go_Advance();
		analogWrite(&ENA_pin, SPEED);
		steer(SLIGHT_LEFT);
//		SendStringUSB("S LEFT");
	}
	if (sensor == 0b00010 || sensor == 0b00011 || sensor == 0b00001) { //The black line is on the right of the car, need right turn
		go_Advance();
		analogWrite(&ENA_pin, HI_SPEED);
		steer(RIGHT);
//		SendStringUSB("RIRGHT");
	}
	if (sensor == 0b00110 || sensor == 0b00111 || sensor == 0b01111) {
		go_Advance();
		analogWrite(&ENA_pin, SPEED);
		steer(SLIGHT_RIGHT);
//		SendStringUSB("S RIRGHT");
	}
	if (sensor == 0b00100 || sensor == 0b01110) {
		go_Advance();
		analogWrite(&ENA_pin, SPEED);
		steer(FRONT);
//		SendStringUSB("FRONT");
...
void StartDefaultTask(void const * argument)
{
  ...
  MX_USB_DEVICE_Init();
  /* USER CODE BEGIN 5 */
	ExecATcmd("AT+CWMODE_CUR=3\r\n");	// softAPmode
	ExecATcmd("AT+CWSAP_CUR=\"OSOYOO_ROBOT\",\"\",10,0\r\n");
	ExecATcmd("AT+CWDHCP_CUR=0,1\r\n");
	ExecATcmd("AT+CIPMUX=1\r\n");
	ExecATcmd("AT+CIPSTART=4,\"UDP\",\"192.168.4.1\",0,8888,2\r\n");// connect UDP
	Bufstrcpy(msgbuf, "start\r\n");
	SendStringUSB(msgbuf);
	int action = 0;
	int turn_flag = 0;
	/* Infinite loop */
	for (;;) {
		...
		while(osSemaphoreWait(myBinarySem01Handle, osWaitForever) == !osOK);
		RecvFirstBee(rxbuf, sizeof(rxbuf));
		osSemaphoreRelease(myBinarySem01Handle);
		int len = strlen(rxbuf);
		if (len > 0) {
			c = rxbuf[len - 1];
		}
		//			snprintf(txbuf, sizeof(txbuf), "packet:=%c\r\n", c);
		//			SendStringUSB(txbuf);
		if (tim_flg == 0) { //setup avoid interrupt
			i2cbuf[0] = 0x10;
			HAL_I2C_Mem_Write(&hi2c1, I2C_ADDR_W, 0x10, I2C_MEMADD_SIZE_8BIT,
					i2cbuf, 1, 1000);
			tim_flg = 1;
		}
		if (c == 'E') {
		...
		if (action == 2)
			auto_avoidance();
		if ((DISTANCE_LIMIT > watch()) && (final_turn == 0)) {
			final_turn = 1;
		}
		osDelay(10); //set 100 passed
	}
  ...
  /* USER CODE BEGIN StartTask03 */
	...
	for (;;) {
		if (osSemaphoreWait(myBinarySem01Handle, osWaitForever) == osOK) {
			if (tim_flg == 1) {
				if (shock_A == 0 && shock_B == 0) {
					short int data[3];
					int total_acceleration = 0;
					read_accelerometer(data);
					total_acceleration = abs((int) data[0]) + abs((int) data[1])
							+ abs((int) data[2]);
					if (total_acceleration > SHOCK_THRESHOLD) {
						if (data[0] > 3000) {
							shock_A = 1;
						}
						if (data[0] < -3000) {
